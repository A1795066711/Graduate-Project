...
class Sorted_relation_and_entity_list_Management(File_Management):
    ...
    def __init__(self, TEST_DATA_DIR, MODEL_OUTPUT_DIR, Competition_Mode=False):
        ...
        # 关系列表 把模型输出的实数值对应为标签
        self.relationship_label_list = ['事故开始时间', '事故影响地区', '事故持续时间', '事故损失', '事故教训', '事故原因', '事故影响人口', 'unknown', '事件主体', '事故发生时间', '事件原因']
#        self.relationship_label_list = [ '事故影响地区', '事故持续时间', '事故损失', '事故教训', '事故原因', '事故影响人口', 'unknown', '事故发生时间']
        self.Competition_Mode = Competition_Mode
    ...
    def produce_output_file(self, OUT_RESULTS_DIR=None, keep_empty_spo_list=False):
        ...
        result_json_write_f = open(out_path, "w", encoding='utf-8-sig')
        count_line_number = 0
        count_empty_line_number = 0
        for text, spo_list in output_dict.items():
            count_line_number += 1
            line_dict = dict()
            line_dict["text"] = text.replace('=', ' ').replace( '@' , '' )
            line_dict["spo_list"] = spo_list
            for a in spo_list:
                print( a )
                a[ 'object' ] = a[ 'object' ].replace('=', ' ').replace( '@' , '' )
                a[ 'subject' ] = a[ 'subject' ].replace('=', ' ').replace( '@' , '' )
                print( a )
