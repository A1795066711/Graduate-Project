...
def replace_empty_infer_predicate_to_three_possible_values(predicate_score_value):
    label_list = ['事故开始时间', '事故影响地区', '事故持续时间', '事故损失', '事故教训', '事故原因', '事故影响人口', 'unknown', '事件主体', '事故发生时间', '事件原因']
#    label_list = [ '事故影响地区', '事故持续时间', '事故损失', '事故教训', '事故原因', '事故影响人口', 'unknown', '事故发生时间']
...
def prepare_data_for_subject_object_labeling_infer(predicate_classifiction_input_file_dir,
    ...
    predicate_score_value_file = open(os.path.join(new_epochs_ckpt_dir, "predicate_score_value.txt"),
                                      "r", encoding='utf-8-sig').readlines()
    output_text_file_write = open(os.path.join(out_file, "text_and_one_predicate.txt"), "w", encoding='utf-8-sig')
    output_token_in_file_write = open(os.path.join(out_file, "token_in_and_one_predicate.txt"), "w", encoding='utf-8-sig')
    output_token_in_not_UNK_file_write = open(os.path.join(out_file, "token_in_not_UNK_and_one_predicate.txt"), "w", encoding='utf-8-sig')
    count_line = 0
    count_empty_line = 0
