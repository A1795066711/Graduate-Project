...
def load_result(predict_filename):
    result_dict = {}
    with open(predict_filename , encoding='utf-8-sig') as gf:
        for line in gf:
            ...
            for item in spo_list:
                o = del_bookname(item['object'])
                s = del_bookname(item['subject'])
                spo_result.append((o, item['predicate'], s))
            spo_result = set(spo_result)
            result_dict[sent] = spo_result
    return result_dict

def load_result_(predict_filename):
    print( predict_filename )
    result_dict = {}
    texts , spo_lists = data.csv_( predict_filename )
    for i in range( len( texts ) ):
        sent = texts[ i ]
        spo_list = spo_lists[ i ]
        spo_result = []
        for item in spo_list:
            o = item[ 2 ].replace( '\xa0' , ' ' ).lower()
            s = item[ 0 ].replace( '\xa0' , ' ' ).lower()
            spo_result.append( ( s, item[ 1 ], o ) ) 
        spo_result = set(spo_result)
        result_dict[sent] = spo_result
    return result_dict
...
def calc_pr(predict_filename, golden_filename):
    ...
    ret_info = {}
    # load test dataset
    golden_dict= load_result_(golden_filename)
    # load predict result
...
if __name__ == '__main__':
    ...
    golden_filename = os.path.join(
        os.path.abspath(os.path.join(os.path.dirname(__file__), "../../raw_data")), "dev.csv")
