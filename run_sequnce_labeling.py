...
class SKE_2019_Sequence_labeling_Processor(DataProcessor):
    ...
    def get_examples(self, data_dir):
        with open(os.path.join(data_dir, "token_in.txt"), encoding='utf-8-sig') as token_in_f:
            with open(os.path.join(data_dir, "token_label_and_one_prdicate_out.txt"), encoding='utf-8-sig') as token_label_out_f:
    ...
    def get_test_examples(self, data_dir):
        with open(os.path.join(data_dir, os.path.join("test", "token_in_and_one_predicate.txt")), encoding='utf-8-sig') as token_in_f:
            token_in_list = [seq.replace("\n", '') for seq in token_in_f.readlines()]
    ...
    def get_predicate_labels(self):
        return ['事故开始时间', '事故影响地区', '事故持续时间', '事故损失', '事故教训', '事故原因', '事故影响人口', 'unknown', '事件主体', '事故发生时间', '事件原因']
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        with open(token_label_output_predict_file, "w", encoding='utf-8-sig') as token_label_writer:
            with open(predicate_output_predict_file, "w", encoding='utf-8-sig') as predicate_predict_writer:
                with open(predicate_output_probabilities_file, "w", encoding='utf-8-sig') as predicate_probabilities_writer:
                    num_written_lines = 0
